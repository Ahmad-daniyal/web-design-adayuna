  ...
  function bindJournalForm() {
    ...
    if (entryForm) {
      entryForm.addEventListener('submit', (e) => {
        ...
        if (!text) {
          Auth.showToast('Catatan tidak boleh kosong', 'error');
          return;
        }

        const mapelColors = {
          matematika: { bg: 'var(--primary-light)', color: 'var(--primary)' },
          fisika: { bg: '#fce7f3', color: '#be185d' },
          kimia: { bg: '#dbeafe', color: '#1d4ed8' },
          biologi: { bg: '#dcfce7', color: '#15803d' },
          sejarah: { bg: '#fef3c7', color: '#b45309' },
          bahasa: { bg: '#ede9fe', color: '#7c3aed' },
          ips: { bg: '#ffedd5', color: '#c2410c' },
          umum: { bg: '#e2e8f0', color: '#475569' }
        };
        const c = mapelColors[mapel] || mapelColors.umum;

        const entry = document.createElement('div');
        entry.className = 'progress-card';
        entry.innerHTML = `
          <div class="flex items-center justify-between mb-2">
            <span class="text-xs font-semibold px-2 py-1 rounded-full" style="background:${c.bg};color:${c.color};">${mapel.charAt(0).toUpperCase()+mapel.slice(1)}</span>
            <span class="text-xs" style="color:var(--text-muted);">Baru saja</span>
          </div>
          <p class="text-sm leading-relaxed" style="color:var(--text-secondary);">${text}</p>
